...
public class GraphicalModel extends AbstractBlastJob{
	...
	public Map<String, String> getExtraJobProperties() throws NoSuchJobException {

		Map<String, String> props = Maps.newTreeMap();
		if(StringUtils.isNotBlank(db)) {
			props.put("DATABASE_KEY", db);
		}

		if(StringUtils.isNotBlank(blast)) {
			props.put("APPLICATION_KEY", blast);
		}

		if(StringUtils.isNotBlank(output)){
			props.put("OUTPUT_FILE_KEY", output);
		}

		if(StringUtils.isNotBlank(start)) {
			props.put("QUERY_LOC_KEY_START", start);
		}
		if (StringUtils.isNotBlank(stop)) {
			props.put("QUERY_LOC_KEY_STOP", stop);
		}
	...
	private void setWordSize(String size) throws NumberFormatException, IllegalArgumentException {
		...
		if(Integer.parseInt(size) >= 2) {
			...
			throw new IllegalArgumentException("word size is less than 2");
		}
	}
}